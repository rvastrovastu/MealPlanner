...
app.post("/api/ai/meal-swap", (req, res) => {
  const { mealName, dietType, goal } = req.body || {};

  res.json({
    originalMeal: mealName || "Selected Meal",
    swaps: [
      {
        ...
        fat: 12,
        fiber: 9,
        reason: `High-protein alternative suitable for ${dietType || "balanced diet"}`
      },
      {
        ...
        fat: 14,
        fiber: 12,
        reason: "Good fiber, balanced carbs, family-friendly"
      },
      {
        ...
        fat: 10,
        fiber: 8,
        reason: `Good option for ${goal || "healthy eating"}`
      }
    ]
  });
});

app.post("/api/ai/nutrition-coach", (req, res) => {
  const { question, profile } = req.body || {};
  const firstName = profile?.firstName || "there";

  res.json({
    answer: `Hi ${firstName}. For "${question || "What should I eat?"}", focus on balanced protein, fiber, vegetables, whole grains, and portion control.`,
    suggestions: [
